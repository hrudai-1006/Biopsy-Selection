...
def main():
    ...
    parser.add_argument('--data_dir', type=str, required=True, help="Path to ProstateX dataset directory")
    ...
    parser.add_argument('--patient_id', type=str, required=True, help="Patient ID to evaluate")
    args = parser.parse_args()

    patient_id = args.patient_id
    # Structured output directory
    patient_output_dir = os.path.join(args.output_dir, "prostatex", f"patient_{patient_id}")
    os.makedirs(patient_output_dir, exist_ok=True)

    print(f"Initializing environment for patient {patient_id} using data from {args.data_dir}...")

    # We will initialize the env directly with the real dataset
    env = HistoEnv(
        tile_size=64, result_path=patient_output_dir,
        mode="prostatex", prostatex_data_dir=args.data_dir
    )

    # Set patient ID and load
    ...
    # Get original MRI slice for display
    # env.dataset.get_patient_data returns (volume, targets, img)
    # img is the original SimpleITK image, but for visualization we need the numpy array of it
    # We can get the original un-normalized image by reading it directly again, or we can use the normalized one from volume

    original_mri_path = os.path.join(args.data_dir, f"ProstateX-{patient_id}.nii")
    if os.path.exists(original_mri_path):
        sitk_img = sitk.ReadImage(original_mri_path)
        original_mri = sitk.GetArrayFromImage(sitk_img)
    else:
        # try .gz
        sitk_img = sitk.ReadImage(original_mri_path + ".gz")
        original_mri = sitk.GetArrayFromImage(sitk_img)
